...
func TestClusterIpServices(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			"annotated ClusterIp services with set targetAnnotationKey return an endpoint with annotation target",
			"",
			"",
			"testing",
			"foo",
			...
			[]string{},
			[]*endpoint.Endpoint{
				{DNSName: "foo.example.org", Targets: endpoint.Targets{"this.is.a.test"}},
			},
			false,
		},
		{
